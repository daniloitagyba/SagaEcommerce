...
## Addendum (2026-08-08): both remaining items closed

**The JWT-bearer wiring duplicated across four `Program.cs` files is deduplicated.** `BuildingBlocks.WebAuthentication`'s `KeycloakJwtBearerExtensions.AddKeycloakJwtBearer(configuration, audience)` is the one copy now - `Authority` resolution, `RequireHttpsMetadata = false`, and the `realm_access.roles` → `ClaimTypes.Role` unpacking all live there once. Each of Orders.Api, Catalog.Service, Inventory.Service and Cart.Service calls it with only the one thing that ever differed between them, their own audience string, and keeps its own `AddAuthorizationBuilder`/policy list untouched - only the authentication half was ever actually identical. Verified with a real `docker build` of all four service images against the real Ubuntu Docker host this addendum was done on, not just a `dotnet build`: the new project resolves and publishes correctly through each Dockerfile's existing selective-`COPY` staging, which needed the same one-line addition in all four.

**`scripts/cart-redis-durability-test.sh` is rewritten**, not left broken. It no longer drives Cart.Service's HTTP API at all - it writes the same Redis Hash shape `CartStore.UpsertItemAsync` writes (field = Sku, value = JSON, `__version` field, TTL) directly through the `redis` container's own `redis-cli`, exactly the shape the original report suggested. That's a closer match to what the script has always actually been measuring - Redis's own survival of a kill under a given persistence mode - than minting synthetic Keycloak tokens for hundreds of shoppers just to reach the same Redis keys indirectly would have been. Smoke-tested against a real Redis container on the same Ubuntu host (`--save "" --appendonly no`, matching mode `none`): the pre-kill `HGET` returns the write, the container answers `PING` after a `SIGKILL` and restart, and the post-kill `HGET` correctly comes back empty - the expected loss for that mode, not a script bug.

## See also

- [Milestone 26: AuthN/AuthZ and Zero-Trust](milestone-26-authn-authz.md) — the JWKS-backed `AddJwtBearer` pattern and `realm_access.roles` unpacking this milestone copied into three more services, and the addendum above collapsed back into one.
...
- [Milestone 46: Redis Durability for Cart.Service, Measured Not Assumed](../data/milestone-46-redis-durability.md) — the original `cart-redis-durability-test.sh` and the persistence-mode results this addendum's rewrite still reproduces the mechanics of.
