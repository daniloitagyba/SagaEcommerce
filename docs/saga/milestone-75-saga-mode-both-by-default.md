...
> **Superseded.** The default documented here (`Both`, set in `compose/compose.yaml` and every `kubernetes/base/*.yaml` manifest) was later changed to `Orchestration` in commit `cfe528f` ("harden distributed workflows and service boundaries"), and that is the value deployed today - both files, and the code's own default in `Orders.Worker/Program.cs` and `Payments.Service/Program.cs`, currently read `Saga__Mode: Orchestration`. `Both` remains useful for a side-by-side comparison (temporarily override it), but is not what a fresh `docker compose up` or `kubectl apply` runs. This milestone's own reasoning for *why* `Both` beats plain `Orchestration` - keeping the choreographed path exercised - still holds; only the deployed default moved.
