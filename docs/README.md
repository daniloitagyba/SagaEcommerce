...
- [Milestone 75: Saga:Mode=Both Is the Default Now, Not Choreography](saga/milestone-75-saga-mode-both-by-default.md) — superseded: the deployed default is `Orchestration` as of `6118979` (single-sourced via `saga-config`/the `x-saga-mode` anchor); this report is a historical record of what was true when written, not current behavior
...
- [Milestone 78: Every Line Gets Reserved Now, Not Just the Biggest One](saga/milestone-78-multi-line-saga.md)
