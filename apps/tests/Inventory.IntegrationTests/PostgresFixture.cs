...
/// PostgresCollectionDefinition, instead of the 6 each test class used to start (and
...
/// so unqualified SQL resolves into that schema without any query needing
/// to change. The name passed in gets a Guid suffix appended before it
/// becomes the schema name - xUnit gives every [Fact] its own fresh
/// instance of the test class, and each of those instances calls this once
/// from InitializeAsync, so a class with several [Fact]s needs one fresh,
/// empty schema per instance, the same isolation each one got from its own
/// dedicated container before this change.
/// </summary>
